...
class Economy(commands.Cog):
    ...
    async def balance(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=f"Balance for {interaction.user.name}",
            description="Current balance for user",
            color=discord.Color.blue(),
        )
        embed.add_field(
            name="Balance", value=data[str(member)]["balance"], inline=False
        )
    ...
    @app_commands.command(
        name="restoreallbalances", description="restore values to all balances"
    )
